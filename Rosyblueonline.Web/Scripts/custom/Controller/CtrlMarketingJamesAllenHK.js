var CtrlMarketingJamesAllenHK = function () {
    var objSvc = null; $frmJamesAllenHK = null, dtMarketing = null;
    var valJamesAllenHK = null;
    ...
    var RegisterEvent = function () {
        $(document).on('click', '#btnSave', function (e) {
            e.preventDefault();
            if ($frmJamesAllenHK.valid()) {
                var pData = ReadForm();
                objSvc.CreateJamesAllenHK(pData).then(function (data) {
        ...
        $(document).on('click', 'a.btnEditDetails', function (e) {
            ...
            objSvc.EditJamesAllenHK(id).then(function (data) {
                if (data != null) {
                    $('#hfJamesAllenHK').val(data.SrNo);
                    ...
                    $('input[name=JamesAllenHK_Status][value=' + data.Isactive + ']').prop('checked', true);


                    $('#btnUpdate').show();
                    $('#btnSave').hide();
                    $('#JamesAllenHKStatus').show();


                } else {
                    $('#btnUpdate').hide();
                    $('#btnSave').show();
                    $('#JamesAllenHKStatus').hide();
        ...
        $(document).on('click', '#btnUpdate', function (e) {
            ...
            if ($frmJamesAllenHK.valid()) {
                ...
                objSvc.UpdateJamesAllenHK(pData).then(function (data) {
                    if (data.IsSuccess && data.Result > 0) {
        ...
        $(document).on('click', 'a.btnDelete', function (e) {
            ...
            uiApp.Confirm('Confirm to delete this record ?', function (resp) {
                if (resp) {
                    objSvc.DeleteJamesAllenHK(id).then(function (data) {
                        if (data != null) {
    ...
    var ClearForm = function () {
        $('#frmJamesAllenHK').find("input[type=text], textarea").val("");
    }
    ...
    var UpdateReadForm = function () {
        return {
            ...
            haVgDisc: $('#txtHaVgDisc').val(),
            CNRDiscHA: $('#txtCNRDiscHA').val(),
            SrNo: $('#hfJamesAllenHK').val(),
            Isactive: $('input:radio[name=JamesAllenHK_Status]:checked').val()
        ...
        if (dtMarketing.getDataTable() == null || dtMarketing.getDataTable() == undefined) {
            dtMarketing.init({
                ...
                dataTable: {
                    ...
                    ajax: {
                        type: 'Post',
                        url: '/Marketing/GridJamesAllenHK',
    ...
    var SetValidation = function () {
        $frmJamesAllenHK = $('#frmJamesAllenHK');
        var rPattern = /^100(\.0{0,2}?)?$|^\d{0,2}(\.\d{0,2})?$/;
        valJamesAllenHK = $frmJamesAllenHK.validate({
            keypress: true,
            onfocusout: false,
            rules: {
                caratRange1Disc: {
...
$(document).ready(function () {
    CtrlMarketingJamesAllenHK.init();
});
